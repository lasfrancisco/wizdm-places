...
export class RouteObserver implements CanActivate {
  ...
  public register(...actions: string[]): Observable<ActivatedRouteSnapshot> {

    return this.observers.pipe( filter( request => actions.some( action => request.data.action === action )) );
  }
}